...
const nameFields = ({label,meta:{error,touched},input:{onChange}})=>{
    return(
        <View >
            <FormLabel  labelStyle={styles.formLabel}> {label} </FormLabel>
            <FormInput   underlineColorAndroid={'transparent'} maxLength = {40}  inputStyle={styles.inputBox} onChangeText={onChange}/>
            {touched && (error && (<Text style={{color:'red',textAlign:'center'}}>{error}</Text>))}
        </View>
    )
};
const messageFields = ({label,meta:{error,touched},input:{onChange}})=>{
    return(
        <View>
            <FormLabel  labelStyle={styles.formLabel}> {label} </FormLabel>
            <FormInput  underlineColorAndroid={'transparent'}  maxLength = {100}  multiline = {true}  numberOfLines = {4} inputStyle={styles.inputBox} onChangeText={onChange}/>
            {touched && (error && (<Text style={{color:'red',textAlign:'center'}}>{error}</Text>))}
        </View>
